...
**Phase 3b, `infra` project, partial (2026-09-02).** Five spec files under `test/e2e/specs/boot/`, covering 8 of the 9 bootstrap test IDs: `boot-fingerprint.infra.spec.ts` (E2E-BOOT-01, -05, -06, one file since the three share step-ca's identity across the job, per Section 3.1's teardown table), `boot-ca-down.infra.spec.ts` (E2E-BOOT-02, -09, the second case swapping in `E2E_CA_DOWN_FINGERPRINT_OVERRIDE` at runtime the way `scenario.sh` intends), `boot-provided.infra.spec.ts` (E2E-BOOT-03, seeding an operator cert into the `step-ui-ssl` volume via a throwaway alpine container before step-ui's first start), `boot-selfsigned.infra.spec.ts` (E2E-BOOT-04), and `boot-fatals.infra.spec.ts` (E2E-BOOT-07's five cases plus its positive control). Helper additions in `helpers/compose.ts`: `upStack` (a longer timeout than `compose()`'s default 120s, since a cold `--build` runs past it), `containerId`/`inspectState`/`waitExited` (for E2E-BOOT-07's exit-code read), `projectName`/`resetStepUIVolumes` (drops step-ui's own volumes between fingerprint-scenario cases while leaving `step-ca-data` alone). `helpers/env.ts`'s `HOST_URL` constant was replaced with a `hostURL()` function: the published port only exists in the scenario's copied `.env` file, never in the harness's own environment, so the old constant read the wrong thing (unused until now, since nothing called it before this pass).

Verified with real `make e2e-bootstrap SCENARIO=<name>` runs on 2026-09-02: `fingerprint` (3 tests, 1.6m), `ca-down` (2 tests, 59.8s), `provided` (1 test, 29.1s), `selfsigned` (1 test, 17.0s), all green. `fatals` (E2E-BOOT-07) is marked `test.fixme` at the top of its `describe.serial` block, not deleted: a first full run hung because `entrypoint.sh`'s 60x1s wait-for-postgres loop takes far longer than 60s when the `postgres` hostname has no container at all (DNS resolution stalls rather than failing each attempt fast), not just when it's stopped. The fix (bring postgres up healthy for every case, including the four whose own fatal never touches the database) is applied in the file, but only case (a)'s first sub-case was re-confirmed before this pass ended; cases (b) through (e), the second SECRET_KEY sub-case, and the positive control were not re-run after the fix, and case (c)'s own timing (postgres up then deliberately stopped, as opposed to never started) was not re-verified either.

No CI workflow invokes the `infra` project. `lint-meta.yml`'s existing harness `tsc`/`eslint` steps cover the new files; the `e2e-bootstrap` matrix job itself remains unwritten (see Remaining).
...
- `infra` project: E2E-BOOT-01 to -07 and -09 are written (see Phase 3b above); E2E-BOOT-07 is `test.fixme`'d pending re-verification (2026-09-02 note in the file). E2E-BOOT-08 (SIGTERM drains in-flight requests, nightly `bootstrap-extra` leg) is not written. The `e2e-bootstrap` matrix CI job that would actually run these five scenarios is not written either; today they only run locally via `make e2e-bootstrap SCENARIO=<name>`.
