...
**Stop early.** A partial migration is a legitimate end state rather than a failure, but **the clean stop is after Phase 2, not after Phase 4**: at that point the contract and the client pipeline exist and nothing temporary has been built. Section 8 sets out why, and what a Phase 4 stop would additionally cost. **Stopping early is the correct plan if the appetite is smaller than the work**, and it is preferable to compressing all twelve phases.
