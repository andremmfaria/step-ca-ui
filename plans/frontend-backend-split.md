...
Status: revision 10, 2026-09-02. Written 2026-08-13 against `main` at `a549750`, revised through two four-phase reviews, and **partly executed: Phases 0, 1 and 2 are complete and merged** (Phase 1 merged 2026-08-15, Phase 2 merged 2026-09-02, last code commit `d38354c`), so the decisions they tested are now measured rather than proposed. **All seven open questions are answered as of 2026-08-15, so nothing in this plan is blocked on a decision.** The e2e track is active, not paused, with a scope correction recorded in Section 10. Revision 9 folded in a 2026-09-02 maintenance pass (Go 1.26.8, Vite 8.2.2, GitHub Actions pinned by version tag rather than commit SHA, npm added to Dependabot for `frontend`, `clients/ts` and `test/e2e`) and removed the stale `plans/step-cli-to-ca-lib-swap.md` draft, recorded in Section 12. Revision 10 records Phase 2's completion.
...
**The three original open questions were answered on 2026-08-13** and folded into the decisions: the API is versioned even though no second consumer exists yet (5.1), "separate" means two containers with nginx serving the SPA (D1, D5), and there is no deadline (Section 8). **The two-container answer then opened four more (Q4 to Q7), and those were answered on 2026-08-15.** Compose is the only supported topology, but D5 drops the mechanisms that would hard-block an orchestrator later (Q4); the operator is every self-hoster of this repository and is unreachable for questions, so every key ships with a safe default and a README row (Q6); and the breaking-change gate demands a `docs/contract-changes.md` row rather than a MINOR bump (Q5). Q7 was answered in Section 8: the clean stop is after Phase 2.
...
| [3. Decisions and alternatives](#3-decisions-and-alternatives) | D1 to D12. D1, D2, D4, D6 and D7 record rejected options. D3, D5, D8, D9, D10, D11 and D12 are consequences of those, or of an owner instruction |
...
| [8. Phases](#8-phases) | Phases 0, 1, 2, 3a, 3b, 3c, 4 to 9, all on `main`. 0, 1 and 2 are complete, only Phase 9 is irreversible |
| [9. CI and CD](#9-ci-and-cd) | 9.1 Job graph · 9.2 Workflow changes · 9.3 Rollback and recovery |
| [10. Impact on the e2e suite](#10-impact-on-the-e2e-suite) | active, not paused. what survives, and the resumption order |
...
| [12. Open questions](#12-open-questions) | All seven answered. Q1 to Q3 on 2026-08-13, Q4 to Q7 on 2026-08-15 |

---

## 1. Objective

### 1.0 Why, and on what reading

The stated motive was that the current architecture is "not very scalable". Nothing in the repository supports a load reading: no live deployments, no operators, one tenant, one Go process serving 65 routes off one Postgres instance. **This plan proceeds on the development-scaling reading** and the owner has confirmed it (Q1): one typed contract instead of 33 hand-maintained templates and 16 untyped scripts, one declared shape per request and response instead of three POST handlers switching on a form field, and a machine-readable description a second consumer could be generated against. 1.3's non-goal about a public API surface is the one most likely to be reopened, because that second consumer is the strongest argument the split has.

### 1.1 What exists today

**What is being asked for is not a separation, it is a rewrite of the user interface.** The request described moving an existing TypeScript frontend off the Go backend. No such frontend exists to move. Every page is rendered by Go from `html/template`, so nothing is relocated: 33 templates, 16 scripts and every form post are deleted and written again as React components against 70 new JSON operations, and 27 of the 34 written e2e test IDs are rewritten with them and one is retired. The backend keeps its database, session, CA and ACME code, but all 65 of its routes change shape. Twelve phases of work, of which one is irreversible. Size the decision against that, not against the word "separate".
...
- Splitting into two repositories, or serving the SPA from a second origin. See D1.
...
### 1.4 What it buys, what gets worse, and what was rejected

Every acceptance criterion in Section 2 is internal and mechanical. None says why the work is worth doing, so it is stated once here.

**Buys.** A typed contract a reviewer can diff and a compiler can enforce. Three action multiplexers become ten single-purpose operations, each with its own role marker and audit point. Authorisation stops being "it is in the right chi group" and becomes an enumerable table a test asserts. A description the e2e suite and any future automation can be generated against. Deep-linkable table state, per-item certificate import, and correct pagination, which is a live bug today.

**Costs.** First paint needs a JavaScript bundle. The strict CSP means toast, theming and anything else that would normally be a dependency is hand-rolled. 27 e2e specs are rewritten. For six phases the system carries two UIs and two authorisation implementations (R22). Ongoing: two runtime images on two CVE cadences, a third-party base image to pin, a Node build stage no image scanner sees, three new npm ecosystems totalling several hundred transitive packages, an nginx config that is now the security perimeter, and a cross-container certificate reload mechanism that does not exist today. 9.1's claim that two images is a net win holds for CI cache scoping and not for total ongoing work.

**Rejected, and why.** *htmx or Alpine over the existing SSR* delivers no machine-readable contract, which was asked for explicitly, and leaves the untyped `static/js` layer. *A hand-written spec over the existing handlers, with no SPA* gives half the value for a tenth of the cost, but a spec no Go tool checks drifts immediately, which is D2's whole argument. *Fixing the concrete rough edges in place* (the three multiplexers, `json` tags, the pagination order, typing the static JS) leaves no contract, though this plan does all four anyway as side effects of Phases 4 to 8. *Stopping early* is not rejected: Section 8 sets out where the clean stop is.
...
- [ ] `x-required-role` is present on every operation whose `auth` is absent, absent on every `public` and `optional` one, and matches the role golden table for the same `(method, path)`. This is the only cross-check between runtime metadata, which never reaches the spec, and the document.
- [ ] `oasdiff breaking` fails the `client` job when a breaking change lands with no matching row in `docs/contract-changes.md`, proven once by a negative-test commit. The row names the operation, what changed and why, and the gate asserts a row exists for every operation `oasdiff` reports as broken (Q5). A MINOR bump is neither required nor accepted as the remedy.
...
- [ ] The SPA issues no HTTP request except through the generated client. ESLint bans `fetch(`, `XMLHttpRequest`, `axios` and `EventSource` in `frontend/src/**` outside `src/api/client.ts`. **This is what makes the contract guarantee real rather than customary**, and it lands in Phase 3a step 1 before any domain phase writes a call site.
...
- [ ] `TRUST_PROXY=true` **and** `TRUSTED_PROXY_CIDRS` are present in the base compose environment block and in `.env.example`. `TRUSTED_PROXY_CIDRS` is a comma-separated **list**, empty by default, and when empty the backend resolves `WEB_UPSTREAM_HOST` at startup and trusts the resulting `/32` (D5 rule 3, Q4). Setting `TRUST_PROXY` with neither the list nor a resolvable upstream is `log.Fatalf` (`main.go:140-147`), not a degradation. `step-net` declares an explicit `ipam.config.subnet` whose default is outside `172.16/12`, and no service carries a static address.
- [ ] **(Phase 3b)** Every environment key the split introduces has a row in `README.md`'s configuration table with a default and a stated failure mode: `WEB_TLS_MODE`, `WEB_HOSTNAME`, `WEB_UPSTREAM_HOST`, `TRUST_PROXY`, `TRUSTED_PROXY_CIDRS`, `STEP_NET_SUBNET`, `WEB_HSTS_MAX_AGE`, `PUBLIC_BASE_URL` and `IMAGE_TAG`. A CI check greps the table for each key, because the operator is a stranger who cannot ask (Q6).
...
- [ ] Go reads `X-Request-Id` as nginx sets it, mints one when the header is absent (which through the origin never happens, since `proxy_set_header` overwrites any inbound value, so the mint path covers direct-to-backend calls only), uses it as the log correlation id and echoes it on every `/api/v1` response. A test issues one request through the origin and asserts the same id in nginx's access log, the backend's structured log and the response. Without a consumer the header and the log format are decorative.
...
- [ ] `docker inspect step-ui-web` shows `step-ui-web-ssl:ro` as its only certificate-bearing mount. `step-ui-ssl`, which holds the backend's `server.key`, and `step-ca-data` are not mounted, and reading `server.key` from inside the nginx container fails. This is D5 rule 5's key-separation argument made mechanical, with `proxy_ssl_name step-ui`, and an untrusted upstream certificate produces the `@apidown` 503 rather than silently downgrading. The compose default `UI_TLS_MODE` is `stepca`, since a self-signed backend leaf cannot satisfy this.
- [ ] nginx serves its **own** leaf (`web.crt`) carrying the public hostname, and the backend leaf carries `step-ui`. One certificate cannot satisfy both roles, and sharing one private key across both containers is rejected on its own merits (D5 rule 5).
- [ ] `docker compose restart step-ui` and the next request through nginx succeeds with no action on `step-ui-web`, which is the `resolver` plus variable `proxy_pass` behaviour (D5 rule 7).
...
- [ ] `vite.config.ts` declares proxies for `/api`, `/auth`, `/health`, `/ready` and `/openapi.json` with `secure: false` and declares no `server.cors` key. Hot reload is verified by hand in the Phase 3a PR and is not a CI gate.
...
Everything below was verified against the repository. Eleven things that looked obvious turned out to be wrong, so they are stated as numbered rules rather than as advice, and the config is given as the artefact rather than as prose.

The config below is the **Phase 9 end state**. For the migration window Phase 3c substitutes a `/app/` block plus a `location /` passthrough to the backend, every temporary block marked `PHASE9-DELETE`.

```nginx
# frontend/nginx.conf.template. REPLACES /etc/nginx/conf.d/default.conf, which
# must be deleted in the image or the stock port-80 server survives internally.
...
resolver_timeout 2s;   # timeout= is NOT a resolver parameter; nginx refuses to start
...
**Rule 1: `X-Forwarded-For` alone is not enough, and rules 1 and 2 are one coupled defence rather than two.** `clientFromHeaders` (`middleware/realip.go:55-77`) walks XFF right to left, skips every entry that is itself trusted, and **falls through to `X-Real-IP` then `True-Client-IP`** when XFF yields nothing (`realip.go:12`, `:71-74`). Whoever reaches that fallback controls the login rate limiter, `auth_log.ip`, `users.last_ip` and the security log: unlimited password guessing by rotating a header, and the ability to block a colleague by forging theirs.

**Phase 0 measured this rather than reasoning about it, and corrected the mechanism.** An earlier draft said the fallback "fires on every request" because XFF would hold nginx's own trusted address. That is wrong: `$remote_addr` is the *client* as nginx sees it, not nginx itself, so with `X-Forwarded-For $remote_addr` set the right-to-left walk returns the real client immediately and never reaches the fallback. The spike confirmed it, with XFF set and `X-Real-IP` forged the limiter counted down normally.

**The fallback is reached when XFF is absent**, which is precisely rule 2's inheritance failure: a proxying `location` that defines any `proxy_set_header` of its own inherits none of the others. The spike reproduced the vulnerability that way and it is total: five failed logins each carrying a different forged `X-Real-IP` left the counter frozen at "4 attempts remaining", so every forged value got its own bucket. With the full header set restored, the same five attempts counted 4, 3, 2, 1 and then blocked. **The severity stands, the cause moves from the replace form to the missing include**, which makes rule 2 load-bearing rather than tidiness.

The fix is unchanged and is both halves: nginx sets `X-Real-IP`, clears `True-Client-IP` and `Forwarded`, and every proxying block carries the include; **and** Go's `forwardedHeaders` fallback (`realip.go:12`) is **deleted**, not gated, because an opt-in is a config a future edit can set.

**Rule 2: `proxy_set_header` inherits exactly like `add_header`, and the legacy blocks are where it bites.** Directives at one level are inherited only when that level defines none of its own. Phase 3c's legacy passthrough blocks either define a subset and silently lose the rest, or define none and inherit from a server level that defines none. Either way `POST /login`, the one route the rate limiter protects during Phases 3c to 8, would take the client's raw headers. Hence the `include` file above, in every proxying block, with no exceptions.

**Rule 3: the trusted list is resolved from nginx's name, not pinned to an address.** `docker-compose.yml:151-156` declares `step-net` with no `ipam` block, so Docker allocates dynamically and no hardcoded CIDR survives a `docker network rm`. Q4's answer shapes the fix. `TRUSTED_PROXY_CIDRS` is a **list**, empty by default, and when it is empty the backend resolves `WEB_UPSTREAM_HOST` (default `step-ui-web`) through the platform's DNS at startup and trusts the resulting `/32`, so the address is discovered per boot rather than written down. When the list **is** set it is used verbatim and no resolution happens, which is the path a deployment behind an ingress or a load balancer takes without any of this being redesigned. Trusting the subnet is wrong: the bridge gateway sits inside it. `step-net` still gets `ipam.config.subnet` as an env-var default, since `172.16/12` is where corporate VPNs live and a self-hoster who collides with theirs (Q6) needs exactly one key to change, but no service carries a static address. **Adding an upstream hop later is a change to nginx, not to this list**: the mechanism is nginx's realip module (`set_real_ip_from`, `real_ip_header`, `real_ip_recursive on`), after which `$remote_addr` is the true client and the replace form stays correct. Widening `TRUSTED_PROXY_CIDRS` instead collapses the rate limiter to one bucket for the whole internet.
...
The Go container also **copies `cfg.RootCert` into the `step-ui-web-ssl` volume as `root_ca.crt`** at the end of the bootstrap. nginx therefore mounts exactly one volume, `step-ui-web-ssl:ro`, carrying `web.crt`, `web.key` and `root_ca.crt`, and never gets read access to `step-ui-ssl` (which holds the backend's `server.key`, the credential for impersonating the backend) or to the CA's `/home/step`. The fingerprint scenario relocates the root CA on `step-ca-data`, which the copy step is upstream of, so `compose.e2e-fingerprint.yml` needs no volume edit.
...
**Independent rollback needs image tags, which compose does not have today.** Both application services use `build:` with no `image:`, so "roll back one half" is a `git checkout` and a rebuild, which the single image could already do. Both gain `image:` alongside `build:`, defaulting to **one shared `${IMAGE_TAG}`** so an accidental partial upgrade is impossible, with `${BACKEND_IMAGE}` and `${WEB_IMAGE}` as per-half overrides so a deliberate one is possible. `docker-build.yml` tags both with the commit sha. Without that indirection the deployment split buys nothing operationally over one image.
...
| `.github/workflows/security.yml` | four distinct sites: gosec's `./step-ui-go/...` plus its setup-go pair, govulncheck's `working-directory` plus its setup-go pair, and `trivy-image`'s `docker build -f step-ui-go/Dockerfile step-ui-go`, which is a **Docker context** and therefore also needs Phase 3c's context widening |
...
│   └── static/                      DELETED in Phase 9 (css is copied, not moved, in Phase 3a)
...
**Four containers**, up from three. `postgres` and `step-ca` are unchanged. `step-ui` keeps its name (D11) and stops publishing a host port. A new `step-ui-web` runs nginx, publishes the host port, and is the only service the browser or the e2e harness ever addresses.
...
Volumes: the six existing ones plus a new `step-ui-web-ssl`, read-write to `step-ui` and read-only to `step-ui-web`, carrying `web.crt`, `web.key` and `root_ca.crt`. `step-ui-ssl` is not mounted into nginx at all (D5 rule 5). Compose gains `step-ui-web depends_on: step-ui: condition: service_healthy`, and the healthcheck inside the Go container stays where it is, corrected to `curl -fsk https://localhost:8443/health` (it currently curls `/login` without `-f`), with a second, simpler one on nginx.
...
Two consequences of nginx owning the first hop. The SPA fallback is `try_files` in nginx, not Go, so the earlier design's "the fallback must not swallow `/api/`" trap disappears: nginx proxies those paths and never falls back for them. And `RealIP` only produces the true client address because nginx sets `X-Forwarded-For` **and** `X-Real-IP` to `$remote_addr`, clears `True-Client-IP` and `Forwarded`, `cfg.TrustProxy` is on with a `/32` list, and Go's `forwardedHeaders` fallback is deleted. D5 rule 1 explains why all four are mandatory.
...
- **OIDC does not traverse the Vite proxy.** The issuer redirects the browser to the registered redirect URI, which comes from `OIDC_REDIRECT_URL`, **not** `PUBLIC_BASE_URL` (D5 rule 10). Without `OIDC_REDIRECT_URL=http://localhost:5173/auth/oidc/callback` in the dev environment and a matching redirect URI registered at the issuer, the callback lands on the backend origin and is served the placeholder.
...
dev:       ## Compose backend up (with --wait), Vite on host with hot reload
	$(COMPOSE) up -d --wait postgres step-ca step-ui
...
**The operative rule.** Breaking changes land in `v1` in lockstep, because both artefacts are built from one commit. `oasdiff breaking` is a review signal that such a change is happening, plus the prompt to re-ask whether an external consumer now exists, and Section 2's `docs/contract-changes.md` criterion is what it enforces (Q5). **The path segment bumps when, and only when, a consumer exists that this repository does not build in the same CI run.** That is the trigger, and it is also the trigger for reopening 1.3's non-goal and D10's aggregates.

**Two things are done now because they are cheap now and expensive later.** `/api/v1` exists once, as a Go constant, and every scope derives from it: today it would be a string literal in the CSRF middleware's scope (5.4), `MaxBytesReader`'s wrap (5.7), Go's CSP scoping (D5), `X-Session-Expires-At`'s emission scope (5.3) and the 404 handler, plus `nginx.conf` and `vite.config.ts`. That is a correctness defect independent of versioning. The role golden table's `version` column is **dropped** (Q5). Its only purpose was letting a `v1` operation and its `v2` successor carry different roles, no `v2` can exist without the deferred work below, and a column holding one constant value in every row is noise a reviewer learns to skip past. The table keys on `(method, path)`. Adding the column back is the first line of the v2 work, which the next paragraph already prices.
...
| QueryClient defaults | One client with `retry: (n, e) => n < 2 && !isClientError(e)`, `refetchOnWindowFocus: false` (5.3), `staleTime: 30_000`, `throwOnError: false` so query errors are values and error boundaries only catch render crashes. `getSession` overrides to `retry: (n, e) => n < 3 && isNetworkOrServerError(e)`, never on a 4xx (5.10). |
...
### 5.11 One operation, end to end

Every mechanism in this document is described in its own section and none of them is shown meeting the others. This is `getCertificate` traversing all of them. It is the shape the other sixty-nine copy, and it is the fastest way to check that a change to any one section still composes.

**1. The DTO pair, in `apitypes/`.** Hand-written field by field, no `models` type in sight (D3 rule 3, enforced by `depguard`).

```go
type GetCertificateInput struct {
	ID int64 `path:"id" doc:"certificate id"`
}

type GetCertificateOutput struct {
	Body CertificateDetail
}
...
type CertificateDetail struct {
	...
	ExpiryStatus string    `json:"expiryStatus" enum:"ok,warning,expired"`
}
```

`expiryStatus` is server-computed because it retires `templateFuncs.badgeClass`, and the threshold behind it is published as `expiringSoonDays` in `GET /api/v1/config` rather than baked into the schema, which is D3 rule 1.

**2. Registration, in `api/certificates.go`.** `roleOp` is the only way a role is written, and it derives all three representations from one argument (5.5).

```go
huma.Register(api, roleOp("viewer", huma.Operation{
	OperationID: "getCertificate",
	Method:      http.MethodGet,
	Path:        apiV1 + "/certificates/{id}",   // apiV1 is the single constant, 5.1
	Tags:        []string{"certificates"},
	Summary:     "Fetch one certificate",
}), a.getCertificate)
```

**3. The row it must have in the role golden table**, or the role-matrix test fails the build:

```
GET  /api/v1/certificates/{id}  auth=required  role=viewer  tag=certificates  csrf=no  ratelimit=no  templateRouteRetired=false
```

`templateRouteRetired` stays `false` until Phase 9 deletes `GET /certificates/{id}`, and the count of `false` rows reaching zero is a Phase 9 criterion (R22).

**4. What lands in the committed spec**, and therefore in the drift-gate diff a reviewer reads:

```json
"/api/v1/certificates/{id}": { "get": {
  "operationId": "getCertificate",
  "tags": ["certificates"],
  "x-required-role": "viewer",
  "security": [{ "session": ["viewer"] }],
  "responses": { "200": {...}, "401": {...}, "403": {...}, "404": {...}, "422": {...} }
}}
```

`x-required-role` is the only role information that survives into the document, because `Metadata` is `yaml:"-"` (5.5). The 422 arrives free from huma.

**5. The generated client signature**, which no one writes by hand:

```ts
export const getCertificateOptions: (opts: { path: { id: number } }) =>
  UseQueryOptions<CertificateDetail, ErrorModel>
```

**6. The call site**, obeying 5.9 in three ways at once: no hand-assembled query key, no bare `fetch`, and an invalidation tag rather than a literal.

```tsx
const { data, isPending, error } = useQuery(getCertificateOptions({ path: { id } }))
if (isPending) return <Skeleton />
if (error) return <ErrorState problem={error} />
// a mutation elsewhere on this page ends with:
invalidate('certificates', 'dashboard')   // dashboard because D10 rule 1 makes it derivative
```

**What breaks if a link in that chain is skipped.** Return `Body *models.User` at step 1 and a password hash ships (R15). Omit `roleOp` at step 2 and the middleware denies by default, which is the correct failure. Omit step 3 and the build fails. Skip step 6's tag and the dashboard silently serves stale counts after a mutation, which is the one failure in the chain that nothing catches automatically.
...
| n/a | public | `GET /api/v1/config` | **new.** `{oidcEnabled, oidcButtonLabel, acmeEnabled, appVersion, contractSha, roleLevels, sessionIdleTimeoutSeconds, expiringSoonDays}`. `templates/login.html:24` gates the OIDC button on a server-side flag today and the endpoint map otherwise exposes it nowhere, leaving "click and get a 404" as the only discovery mechanism. The SPA hides navigation and login affordances from this and never hardcodes a feature flag or a role ordering |
...
| n/a | `GET /api/v1/admin/backups/{id}/download` (**new**: octet-stream, the target `downloadUrl` names, constrained by 5.7's pattern) |
...
**Count: 70 JSON operations**, plus 5 unversioned Go routes (`/health`, `/ready`, `/auth/oidc/login`, `/auth/oidc/callback`, `/openapi.json`). There is no Go-side SPA fallback and no Go-side asset handler: nginx owns both (D5). Every one of the 65 method-registered routes in `main.go` appears in exactly one row, and no row names a route that does not exist. The `/static/*` handler at `main.go:337` is deleted in Phase 9, not replaced: nginx serves the SPA's assets.
...
**A second gate on breaking changes.** `oasdiff breaking` between the base's `openapi.json` and the pull request's, failing if a breaking change lands without a matching row in `docs/contract-changes.md` (Q5). The row is `| operationId | what changed | why |`, the file is append-only, and the gate asserts a row exists for every operation `oasdiff` names, so the remedy is a sentence a reviewer reads rather than an integer no resolver reads. This is the only mechanism that turns "remember to bump" into something enforced, and it gives reviewers a far better signal than the raw JSON diff. Three mechanics to specify rather than assume: `oasdiff` is not preinstalled on runners and must be **pinned to a release**, not `@latest`, since every `uses:` in this repository pins to a version tag (not a commit SHA, as of the 2026-09-02 maintenance pass) and an unpinned tool install would be the only exception to that discipline. Obtaining the base document needs `fetch-depth: 0`. And the base ref differs by trigger: on `pull_request` it is the PR's actual base ref, which is not necessarily `main`, while on `push: main` there is no base branch and the comparison is against `github.event.before`.
...
**Rough size, and the cheapest useful stopping point.** No estimate here is a commitment and Phase 0 recalibrates all of them, but the shape is decision-relevant now. Phases 0 to 3c are contract and scaffolding: high risk, low volume, and they produce **no user-visible change at all**. Phases 4 to 8 are the UI rewrite: low risk, high volume, scaling with 33 templates rather than with anything in this document. Phase 9 is small. Expect roughly a third of the calendar in Phases 0 to 3 and the rest in 4 to 8, with Phase 4 costing more per operation than 5 to 8 because R23's handler-splitting work is front-loaded and unpriced. **There is no deadline**, which the owner has confirmed. That removes schedule pressure and replaces it with a different risk: with nothing forcing a finish, a half-migrated tree can quietly become the permanent state (R18).

**There are two legitimate end states, and Phase 4 is not one of them.** An earlier draft offered Phase 4 as a clean stopping point, which contradicted R27's own description of everything that makes Phase 4 reachable. The correction:

**Stopping after Phase 2 is clean.** The contract exists, the client pipeline is proven, the drift gate is a required check, and no user-visible change has shipped. Nothing temporary has been built: no `/app/` base path, no duplicated CSS, no legacy proxy blocks, no second authorisation implementation. The old UI is untouched, and the work has bought a machine-readable description of the system that the e2e suite and any future consumer can be generated against. That is the point at which appetite should be re-tested.

**Completing Phase 9 is the other one.** Everything between those two points carries scaffolding whose only justification is that it is being removed. Stopping inside that range leaves two user interfaces, two authorisation implementations that Section 6 makes deliberately disagree on revocation and the CA chain downloads, a SPA at a path that is now in bookmarks, and a CSS file that exists twice. That state works, which is exactly why it would persist.
...
7. **Normalise the recorded client address, in this phase rather than per domain**, because both UIs write the audit log simultaneously for six phases. Seven call sites pass `r.RemoteAddr` where the rest of the codebase passes `clientIP(r)`: `handlers/auth.go:203` and `:225`, `handlers/totp.go:128` and `:162`, `handlers/audit.go:20`, `handlers/oidc.go:85`, `handlers/notifications.go:178`. `LogAuth` writes that value into `users.last_ip` (`db/users.go:240`), so successful logins store `host:port` while the rate limiter keys on bare hosts. The consequence is live today and invisible: `handlers/users.go:183`'s `IPBlocked` badge never fires and the `unblock_ip` button is a no-op. It becomes a delivery problem in Phase 6, where `DELETE /api/v1/security/ip-blocks/{ip}` carries a canonical-IP `pattern` that `422`s on every legacy value, so a backfill migration or a normalisation on read ships with it. **That backfill is not `split_part(ip, ':', 1)`**, which an earlier revision proposed: `RemoteAddr` spells an IPv6 peer `[::1]:8080`, so splitting on the first colon rewrites every IPv6 row to `[`. It is two anchored `regexp_replace` calls, one for the bracketed form and one for `a.b.c.d:port`, both leaving an already-bare value untouched so the migration is idempotent.
8. `r.NotFound` and `r.MethodNotAllowed` emit `404` and `405` as `application/problem+json`. There is no Go-side SPA fallback and no Go-side asset handler: nginx owns both (D5).
9. `.gitattributes` merge driver, `make openapi`, `make hooks`, `docs/contract-changes.md` with its header row, and a **standalone** drift gate plus `oasdiff` gate asserting a row exists for every broken operation (Q5), which Phase 2 relocates into the `client` job. 7.2's same-job requirement becomes true when the `client` job exists. Until then a standalone gate is strictly better than none.
...
### Phase 2. Client package pipeline. **COMPLETE, merged 2026-09-02**

**Exit criterion met, verified rather than asserted.** `docs/contract-proof.md` records a real red run and a real green run against a deleted `proof/contract-negative` branch, dispatched manually since `ci.yml` takes no automatic trigger from a non-`main` branch and this repository takes no pull requests. The red run failed `frontend` and `ci-gate` on exactly the intended line, an installed client version missing the commit's short sha, while `client` and every other job stayed green, proving the provenance assertion is load-bearing rather than decorative and, in the same run, that `ci-gate` fails closed when one dependency does. The tag `client-v0.1.0` published `@andremmfaria/step-ca-ui-client@0.1.207-sha.d38354c` to GitHub Packages from the workflow's own `GITHUB_TOKEN`, no personal access token or manual grant needed. Re-running that tag's workflow (`gh run rerun`) stayed green and logged `already published ..., publish is idempotent`, with the package's version count still 1.

**Four decisions were taken during execution that this plan did not specify.**

1. **Tag pattern is `client-v*`.** D8 and 7.5 left the exact pattern open. It was chosen to sit outside `docker-build.yml`'s existing `tags: ["v*"]` trigger, so a client-only tag never starts an unrelated container build, and `ci.yml` gained its own `tags: ["client-v*"]` push trigger to react to it at all.
2. **`clients/ts/package.json` needed a `./package.json` export it did not have.** Without it, D8's own provenance command, `node -p "require('@andremmfaria/step-ca-ui-client/package.json').version"`, throws `ERR_PACKAGE_PATH_NOT_EXPORTED` under Node's exports enforcement. Found by running the command, not by inspection.
3. **`attw --pack` runs `--profile esm-only --ignore-rules internal-resolution-error`.** hey-api's generated code omits file extensions on relative imports, which `moduleResolution: "Bundler"` (this package's own tsconfig) allows and Node's own `node16`/`nodenext` resolver does not. That is a structural property of the generator, not a bug, and this package's only consumer is Vite, a bundler-resolution consumer, so the strict profile's node16 checks were scoped out rather than worked around.
4. **`contract-negative` does not depend on the proof branch existing.** It reproduces the same regression class in a self-contained job on every `workflow_dispatch` and on any push touching `ci.yml`, `clients/ts/**` or `backend/cmd/openapi/**`: build a real client from the current commit, stamp a version missing the commit's sha, and assert that installing it does not falsely match. The one-off proof would otherwise decay the moment nobody remembered it happened.

**Carried forward, and why.** The `frontend` CI job runs only `typecheck` and `build`, not `lint` and `test`: the Phase 0 spike `frontend/package.json` has neither script, since ESLint and Vitest are Phase 3a's tooling step, not Phase 2's. The dead-SDK-surface script and `clients/ts/unused-allowlist.txt` are likewise Phase 3a's, per its own step 1, and were not added here.
...
The highest-risk phase in the plan. It owns R13, R25, R26, R30 and R32 in full.
...
4. The second certificate: `WEB_TLS_MODE`, `WEB_HOSTNAME` with no fallback, `web.crt`/`web.key` into `step-ui-web-ssl`, the root CA copied where nginx can read it, and the `.fallback` marker plus its `/ready` reporting. **`WEB_TLS_MODE=provided` is a first-class mode, not an afterthought** (Q4): nginx reads a leaf the operator mounts from outside, the certificate wait and the renewer are both skipped, and no running container then depends on another container writing a file into a shared read-write volume. `stepca` stays the default, because it is what a self-hoster gets with no configuration at all.
...
8. **The operating document** (Q6). The operator is every self-hoster of this repository and is not reachable for questions, so each key the split introduces ships with a safe default, a stated failure mode and a `README.md` row: `WEB_TLS_MODE`, `WEB_HOSTNAME`, `WEB_UPSTREAM_HOST`, `TRUST_PROXY`, `TRUSTED_PROXY_CIDRS`, `STEP_NET_SUBNET`, `WEB_HSTS_MAX_AGE` (unset by default, since a two-year policy on a name chaining to a private root is a trap with no downgrade path), `PUBLIC_BASE_URL` and `IMAGE_TAG` with per-half rollback. R30's rule about what may live in `nginx.conf` is the section's first line. This lands in Phase 3b rather than Phase 9 because the keys go live the moment nginx takes the published port.

**Exit criterion.** All four forgery headers fail to move the recorded address, through the origin and directly against the backend, with `TRUSTED_PROXY_CIDRS` both empty (resolved) and explicitly set. `WEB_TLS_MODE=provided` serves an externally mounted leaf with the renewer never running. The README key grep passes. `docker compose restart step-ui` and the next proxied request succeeds. A renewal with a low `UI_CERT_DURATION` produces a new served certificate, exactly one reload per cycle, and no restart. A verified proxy hop on a real stack. With step-ca unreachable, the backend self-signs, writes `.fallback`, `/ready` goes non-200 and nginx never starts. `make nginx-lint` green.
...
2. **Compose:** the fourth container on a pinned, env-var-defaulted subnet whose default sits outside `172.16/12`, with **no static addresses** (D5 rule 3); `step-ui-web-ssl` read-only into nginx and read-write into Go, with `step-ui-ssl` **not** mounted into nginx at all so an RCE there cannot read `server.key`; `init: true`; `depends_on: service_healthy`; the backend healthcheck corrected to `curl -fsk .../health`, which today lacks `-f` and so passes on any status; `UI_TLS_MODE` defaulting to `stepca`; the new `WEB_TLS_MODE`, `WEB_HOSTNAME`, `WEB_UPSTREAM_HOST`, `WEB_HSTS_MAX_AGE`, `STEP_NET_SUBNET`, `TRUST_PROXY`, `TRUSTED_PROXY_CIDRS`, `PUBLIC_BASE_URL` and `OIDC_REDIRECT_URL` keys, noting the base file passes **no** OIDC keys today; `image:` tags on both services; the host port moving to `step-ui-web` **on 8443 internally** so only the hostname changes in the harness; `step-ui-web-ssl` added to `make backup`'s volume list; `.dockerignore`; and the four `.gitignore` additions the `.dockerignore` implies but does not cause.
3. **The eight overrides, plus one new one.** `-config`, `-mail` and `-le` are unaffected. `-image` needs a second `image:` pin, which is the one edit that silently defeats the whole override if missed. `-oidc` needs three: `TRUST_PROXY` defaults to false there, `TRUSTED_PROXY_CIDRS` defaults empty, which is no longer a `log.Fatalf` but instead falls through to D5 rule 3's `WEB_UPSTREAM_HOST` resolution, and that name does not resolve in a scenario with no nginx, and `OIDC_REDIRECT_URL` points at the backend on a path that does not match D5 rule 10's published origin. `-fatals`, `-nodeps` and `-fingerprint` need **no edits**, because the new `compose.e2e-noweb.yml` gives `step-ui-web` a never-enabled profile and `scenario.sh` composes it into every scenario, so nginx never exists for the `infra` project.
...
1. Delete `templates/`, `static/js/`, `h.render`, `h.flash`, `h.base`, and `Handler.tmpls`. `templateFuncs` is deleted, its three functions having been retired rather than ported: `badgeClass` became `expiryStatus`, `daysLeft` and time formatting became client-side, `hasRole` became `roleLevels` in `GET /api/v1/config`. Confirm each has a home first.
2. Delete, in one commit because Go will not compile otherwise: the template-route registrations; `//go:embed templates static` (`main.go:40-41`); `staticHandlerFromFS` (`:70-100`) and its `/static/*` registration (`:337`); `mimeByExt` (`:91-96`); the `init()` MIME block (`:101-116`); the `fs.Sub` block (`:332-336`); the five imports that exist only for those (`io`, `io/fs`, `mime`, `path/filepath`, `strings`); the duplicated CSS under `backend/static/`; the `COPY templates/` and `COPY static/` lines in the backend Dockerfile; nginx's legacy passthrough blocks from Phase 3c step 1 (every block carrying the `PHASE9-DELETE` marker); and the now-false comment at `middleware/middleware.go:62-64` explaining a `Cache-Control` override that no longer exists. Unused imports are a compile error, unused package-level identifiers redden `golangci-lint`, so this is one commit rather than a cleanup pass. The role golden table's count of operations whose `templateRouteRetired` is false must reach zero (R22).
3. Delete the three `backend` tool globs from `lint-meta.yml`'s `style` job, replaced by a `frontend/` lint step.
4. **Flip the SPA base path from `/app/` to `/`**: Vite `base`, the router `basename`, `PUBLIC_BASE_URL`, `OIDC_REDIRECT_URL` and the harness `BASE_URL`. `/reset-password` and the OIDC callback landing do not move, having stayed at the root throughout (R31). Delete every block carrying the `PHASE9-DELETE` marker.
5. CSP in `nginx.conf` against the real built bundle: `connect-src 'self'`, `frame-src 'none'`, `worker-src 'none'`, `manifest-src 'self'`, and `img-src 'self' data: blob:` if not already pulled forward by Phase 8. Confirm `@vitejs/plugin-legacy` is not in use, since it emits inline scripts.
6. Re-floor the coverage gate, which the two-sided ratchet has been doing per phase already.
7. Retire `E2E-STATIC-01` rather than repathing it: it asserts `GET /static/css/pages.css` returns 200 with an enforced MIME type against a Go handler that no longer exists. D11's blast-radius row for `specs/config/static-01.api.spec.ts` covers the rename during the migration window only.
8. Update `README.md`, `Makefile` help, `scripts/test_deploy.sh` (which curls `/login` and two `/static/*` paths), and finalise the `plans/e2e-tests.md` appendix.
...
**Corrected 2026-09-02: the e2e track is active, not paused.** A 2026-08-13 note here said the agent had stopped and no further specs were being written against the interface this plan deletes. Commit `c7a96d6` (2026-08-15) landed anyway, adding the three OIDC `api` specs and all four `ui` companions. The freeze did not hold, and nothing about pausing it has been decided since, so this section now describes the suite as it stands rather than as it was frozen.

**Current state** (verified 2026-09-02 by listing `test/e2e/specs/` and grepping its `E2E-*` IDs, not re-derived from the status doc): harness phases 0 to 2 complete. **24 spec files cover 40 of the 78 indexed test IDs**, up from 17 files and 34 IDs: `AUTH-01, -04, -07, -08, -11 to -16, RBAC-01/02, CSRF-01/05, CERT-01 to -04 and -09, PROV-01/02, HIST-01 to -03, SEC-01/02, ADM-01 to -05, BAK-01, HLTH-01 to -06, STATIC-01, CFG-01`. Pass/skip counts were **not re-measured**: the suite needs the full docker compose stack (postgres, step-ca, secrets with the uid-specific `chmod`) that `plans/e2e-implementation-status.md`'s "Local run" section describes, which is out of scope for this docs-only pass. Treat the ID and file counts above as current and the last known pass level (138 passing, 8 skipped, recorded against 34 IDs) as stale.

**Deviation from this section's own resumption order.** Step 4 below says `ui` specs are written last, after Phase 8, once the markup is stable. They were not: all four (`cert-01.ui`, `auth-04.ui`, `auth-11.ui`, `cfg-01.ui`, covering test IDs CERT-01, AUTH-04, AUTH-07, AUTH-11 and CFG-01) were written in `c7a96d6` against the server-rendered markup this plan's Phase 9 deletes. **Consequence:** those four spec files must be rewritten, or retired, when Phases 4 to 8 replace the pages they assert against. Until then they guard the legacy UI only, not the SPA this plan is building toward.
...
| `api` project, the other 30 written IDs | **Rework.** Path, method, encoding, status code and assertion shape all change. Intent survives, body does not. Includes the `api` half of CERT-01 and AUTH-11, which also carry a `ui` spec |
| `ui` project (4 spec files, 5 IDs: CERT-01, AUTH-04, AUTH-07, AUTH-11, CFG-01) | **Already written, against the interface this plan deletes.** See the deviation note above: rewrite or retire per page, in the phase that replaces that page |
...
6 of the 40 written IDs survive untouched (HLTH-01 to -06), 1 (STATIC-01) is retired in Phase 9, and 30 have an `api`-side body that needs rewriting. 5 of those 40 (CERT-01, AUTH-04, AUTH-07, AUTH-11, CFG-01) are the `ui`-shaped IDs above, of which 2 (CERT-01, AUTH-11) are also in the 30 needing an `api`-side rewrite, since both a body rewrite and a UI-shaped rewrite apply to different halves of the same ID. The largest untouched block, the 9 `infra` IDs, is not written yet. The rewrite is a body rewrite, not a re-derivation: `plans/e2e-tests.md` already states what each test asserts.

**What the two-container topology breaks in the harness, beyond the contract rewrite.** This is a separate axis from the contract-bound specs and it lands in Phase 3c, not per domain.
...
| `compose.e2e-fatals.yml:7` | **No edit needed.** `compose.e2e-noweb.yml` gives `step-ui-web` a never-enabled profile and `scenario.sh` composes it into every scenario, so nginx never exists for the `infra` project (Phase 3c step 3) |
| `compose.e2e-nodeps.yml:7` | **No edit needed.** `compose.e2e-noweb.yml` gives `step-ui-web` a never-enabled profile and `scenario.sh` composes it into every scenario, so nginx never exists for the `infra` project (Phase 3c step 3) |
| `compose.e2e-fingerprint.yml:13-19` | **No edit needed.** `compose.e2e-noweb.yml` gives `step-ui-web` a never-enabled profile and `scenario.sh` composes it into every scenario, so nginx never exists for the `infra` project (Phase 3c step 3) |
...
**Pass level, which Section 2's final criterion cites.** Phase 9 does not merge until all of the following hold. The 6 untouched IDs (HLTH-01 to -06) pass unmodified. All 27 contract-bound IDs are rewritten and passing, and STATIC-01 is retired. The 9 `infra` IDs are written and passing. The 4 `ui` browser companions and the CSP smoke spec are written and passing. Skips number 8 or fewer and each carries a linked issue. And the whole run executes against the two images built in the same workflow run.

---

## 11. Risk register

Each risk names a **trigger** and an **owner phase** unless explicitly parked, and the entries added since the two-container change also carry a **status**. Status is the thing that keeps a register readable across a twelve-phase migration: `discharged at Phase N` entries are struck from the reading list rather than re-read forever, and a reader at Phase 5 should only need the `live` ones.

**Discharged at Phase 0's exit** and not worth re-reading afterwards: R4 and R5 (spec purity and determinism), R6 (the CSRF bootstrap, whose residual is 5.4's concurrency rule and already has a test), R9 (binary downloads, proven against a placeholder resource), R12 (the huma handler signature). **Parked, not live:** R3 (no tag is planned), R20 (withdrawn by removing the branch), R24 (a statement of scope, not a risk). **The thirteen that are the register**, and that a reader short of time should read instead of all of it: R13, R15, R16, R17, R18, R22, R25, R27, R30, R31, R32, R33, R34.
...
**R6. CSRF bootstrap. Largely dissolved.** Making `GET /api/v1/session` return `200` with a discriminator removed the anonymous-401-that-must-still-set-a-cookie sequence, whose failure mode was permanent lockout. What remains is 5.4's concurrency rule, enforced by a `retry: false` unit assertion and a cold-jar e2e. *Trigger:* an intermittent 403 on login. *Owner:* Phase 0 step 5.
...
**R8. Coverage, and the silent case.** The visible failure is a red gate. **The failure that matters is invisible: coverage falls and the gate stays green because the floor is far below the real number.** Measured at the Phase 0 rename: **22.9% against a 15% floor**, so the gap is already 7.9 points. Mitigation is Section 2's two-sided gate, which fails when measured coverage exceeds the floor by more than 5 points so the floor ratchets without a human deciding to raise it, but **the floor must be raised to 22 in the same commit that switches the gate on**, or it red-lines on day one against pre-existing coverage rather than against a regression. *Trigger:* any phase PR whose coverage step passes with no floor change. *Owner:* every phase.

**R9. Binary downloads through a generated client.** Blob handling, filename derivation and the backup flow are the awkward corner of every generated client. Proven in Phase 0 against a placeholder resource, not discovered in Phase 4. A hand-written download wrapper in `src/api/` is expected, and lives there as one reviewed exception rather than a pattern. *Trigger:* Phase 0 step 6. *Owner:* Phase 0 step 6.

**R10. Bundle size and the CSP.** `style-src 'self'` is a hard functional constraint on any library injecting a `<style>` tag, not merely a size concern. *Trigger:* the CSP smoke spec, which is what converts this from "someone notices a broken page" into a failing check. *Owner:* Phase 3c step 5.

**R11. Docker build context widening.** Without `.dockerignore`, five `node_modules` trees, `backups/`, `secrets/` and `.env` enter the context: the first destroys caching and the last three would bake credentials into an immutable image layer. *Trigger:* a build-context size assertion in CI, so it is not "CI feels slow". *Owner:* Phase 3c step 2.
...
**R13. The certificate lifecycle now crosses a container boundary, in three ways rather than one.** nginx reads its certificate once at start, so a renewal is invisible until reload. The reload cannot be issued from the Go container without the Docker socket, which is host root held by the container running an allowlisted shell console, so it is a prohibition rather than an inconvenience. And the self-signed fallback, which exists to keep the UI up when the CA is down, guarantees the opposite under `proxy_ssl_verify on`: both healthchecks green, every API call 502ing. Stale-after-renewal was the only part an earlier draft saw. *Mitigation:* D5 rules 6 and 9 in full, plus atomic rename at the source. *Trigger:* the first renewal after Phase 3b, any expiry alarm, and any `docker compose ps` that is all green with a dead UI. *Owner:* Phase 3b steps 4 and 5. *Status:* live.
...
**R25. The proxy layer is a second place to get identity wrong, and the first attempt at it was wrong.** The CRITICAL finding, stated in full in D5 rule 1 and **reproduced in Phase 0**: when a proxying location loses the forwarding headers to nginx's all-or-nothing inheritance, `clientFromHeaders` exhausts its XFF loop and falls through to `X-Real-IP` and `True-Client-IP` (`middleware/realip.go:12,71-74`), handing an unauthenticated caller the login rate limiter, `auth_log.ip`, `users.last_ip` and the security log. Measured, not theorised: five failed logins with rotating forged values left the counter frozen, and the full header set restored a normal countdown to blocked. Three further silent failures in the same layer: `TrustProxy` off collapses every client to one address, `proxy_set_header` inherits all-or-nothing per level so a legacy block without the include takes raw client headers, and a `location` falling through `try_files` for an API path returns `200 text/html` where the client expects a problem document. *Mitigation:* D5 rules 1 to 4, the shared include, deleting the Go-side fallback, and Section 2's four-header forgery table. *Trigger:* any edit to `nginx.conf` or `realip.go`. *Owner:* Phase 3b steps 1, 2 and 6. *Status:* live.
...
**R34. Stopping inside the migration window leaves the scaffolding permanent. Resolved in Section 8.** An earlier draft named Phase 4 a clean end state while R27 called everything that makes Phase 4 reachable temporary scaffolding. Section 8 now resolves this: the clean stop is after Phase 2, and a Phase 4 stop costs a partial Phase 9 on top. *Trigger:* any decision to stop between Phase 3a and Phase 8, or two consecutive phases with no change in the `templateRouteRetired` count. *Owner:* the repository owner. *Status:* live until a stopping point is chosen.

---

## 12. Open questions

**All seven questions this plan has carried are answered.** Q1 to Q3 on 2026-08-13, Q4 to Q7 on 2026-08-15. Nothing blocks any phase. They are recorded here with what each answer changed, because every one of them moved a decision.

**Q1, answered: what did "not very scalable" mean, and is a second API consumer in scope?** No second consumer for now, **but the API is to be versioned anyway**. That is the right call and it is not the same as "no consumer, so no version": versioning now costs a path segment and a policy, while retrofitting one after a consumer exists costs a migration. 5.1 turns the segment from a hedge into a policy with an enforcement mechanism (`oasdiff` in the `client` job) and states what would have to be built if `v2` ever ships. 1.3's non-goal about a public API surface stands for now, and is the one most likely to be revisited.

**Q2, answered: is one deployable acceptable?** No. **Separate means two containers.** D1 and D5 are rewritten around that: a Go image with no static assets, an nginx image carrying the SPA, and nginx as the single public origin doing path-based routing. The critical consequence, spelled out in D1, is that a *deployment* split is not an *origin* split. Because nginx proxies `/api/`, `/auth/`, `/health`, `/ready` and `/openapi.json` to the backend, the browser still sees one origin, so `SameSite=Strict`, the `__Host-` prefixes, the session-bound CSRF and the no-CORS rule all survive unchanged. Splitting to two real origins instead would have cost all four, and this plan treats that as a non-goal rather than an option. The split also makes the plan **smaller** in one place, since `//go:embed static`, the asset handler and the MIME table all leave the Go side, and **larger** in three, which are R13, R25 and R26.

**Q3, answered: appetite and deadline?** None. It is ready when it is ready. That removes schedule pressure and replaces it with the drift risk R18 now describes: with nothing forcing a finish, the coexistence state can become permanent by default. Section 8 answers that by naming Phase 4 completion an explicit end state rather than a stall, so stopping is a decision and not an outcome.

**Reopened by the two-container answer, and answered 2026-08-15.** Q4 to Q7 did not exist before the split, and Q7 was a contradiction the plan created about itself.

**Q4, answered: compose on one host is the only supported topology, but D5 stops short of mechanisms that cannot be undone.** The concern was real: `depends_on: service_healthy`, static addresses on a pinned subnet, `TRUSTED_PROXY_CIDRS` as a single `/32`, `resolver 127.0.0.11`, a shared read-write volume carrying a certificate between two containers and an entrypoint polling for a file another container writes are all compose-specific, and not one of them survives a move to Kubernetes, ECS or Nomad. The answer is neither "compose forever, design for it" nor "build for an orchestrator now". It is that compose is the only topology this repository supports and tests, while the three decisions that would be **expensive to reverse** are made in their portable form today, because each costs nothing extra now.

Three changes follow, and they are the whole of the answer.

1. **The trusted proxy address is resolved, not pinned.** D5 rule 3 is rewritten: `TRUSTED_PROXY_CIDRS` is a list, empty by default, and when empty the backend resolves `WEB_UPSTREAM_HOST` at startup and trusts that `/32`. An explicitly set list is used verbatim, which is exactly what a deployment behind an ingress needs. No service gets a static address, and `ipam.config.subnet` survives only as a collision escape hatch for Q6's operator.
2. **`WEB_TLS_MODE=provided` is a first-class mode**, in Phase 3b step 4 rather than as a later addition. An operator mounts a leaf from outside, the certificate wait and the renewer are skipped, and the cross-container read-write volume becomes the convenience default rather than the only path. That is the single mechanism a secret store would have to replace, and now it does not have to replace anything.
3. **`README.md`'s existing ECS and Kubernetes claims stay true.** They already exist in three places (`CA_ROOT_CERT_PEM`, `DATABASE_URL`, the PostgreSQL wait), and the two changes above are what keeps the split from silently contradicting them.

What is deliberately **not** done: `depends_on: service_healthy` stays, `resolver 127.0.0.11` stays with an env override, and no orchestrator manifests are written. Those are cheap to redo and expensive to speculate about. **This closes D5.**

**Q5, answered: the gate demands a line in `docs/contract-changes.md`, and the golden table's `version` column is dropped.** The premise held. `v1` is permanent by construction, so `oasdiff breaking`'s remedy was editing an integer no resolver reads, which is the exact failure class D8 removed from PATCH. A pull-request label was the cheaper alternative and was rejected: the signal dies with the pull request, cannot be diffed, and is invisible to anyone reading the repository a year later.

**The remedy becomes a row.** `docs/contract-changes.md` is append-only, each row is `| operationId | what changed | why |`, and the gate asserts a row exists for every operation `oasdiff` reports as broken. That makes the remedy a sentence a reviewer reads rather than a number nobody reads, it accumulates into a readable history of every contract break, and it costs one file. It lands in Phase 1 step 9 with the standalone gate and moves into the `client` job with it in Phase 2.

**MINOR stops being gated.** It stays in `openapi/package-version.txt` as a free-running marker of contract era that a human bumps when they judge an era has ended, and nothing asserts it. Section 2's criterion and 7.2's gate are both rewritten off it.

**The `version` column is dropped.** Its stated purpose was letting a `v1` operation and its `v2` successor carry different roles. No `v2` can exist without the deferred work 5.1 already prices, and until then the column holds one constant value in every row, which is noise a reviewer learns to skip past in a table whose entire job is being read carefully. The table keys on `(method, path)`, and restoring the column is the first line of the v2 work rather than a surprise inside it.

**Q6, answered: the operator is every self-hoster of this repository, and they cannot be asked anything.** 1.1's "there are no live deployments" is true of this working copy and false of the project. The repository is a public distributable at `github.com/UncleFi1/step-ca-ui`, `scripts/test_deploy.sh` is a curl-to-bash installer aimed at strangers on clean virtual machines, and `UI_TLS_MODE=provided` already documents an "operator-provided cert". The role the two-container design appeared to create in fact already exists, and it is filled by someone with no access to this document.

**That makes D5's rules justified rather than over-built**, which is the opposite of the reading the question offered. Every rule in D5 is a default a stranger inherits without reading it.

**It also creates one deliverable that no phase produced.** Phase 3b gains step 8: each key the split introduces ships with a safe default, a stated failure mode and a `README.md` configuration-table row, with a CI grep asserting the rows exist. The keys are `WEB_TLS_MODE`, `WEB_HOSTNAME`, `WEB_UPSTREAM_HOST`, `TRUST_PROXY`, `TRUSTED_PROXY_CIDRS`, `STEP_NET_SUBNET`, `WEB_HSTS_MAX_AGE`, `PUBLIC_BASE_URL` and `IMAGE_TAG`. It lands in Phase 3b, not Phase 9, because those keys go live the moment nginx takes the published port and a half-migrated tree is still something someone can install.

**Two defaults are decided by this answer rather than left to the phase.** `WEB_HSTS_MAX_AGE` is unset by default, because a two-year policy on a name chaining to a private root is a trap with no downgrade path and a stranger will not know that. And `STEP_NET_SUBNET` defaults outside `172.16/12`, because that is where corporate VPNs live and a collision presents as an unrelated network failure.

**Q7, answered in Section 8.** The clean stop is after Phase 2, not after Phase 4. What remains open is only whether to take it, which is a question about appetite rather than about the plan, and it can be answered at Phase 2's exit with the contract already in hand.
...
- **Visual scope.** A non-goal (1.3), with one live sub-question deferred to Phase 3a's exit with an evidence rule: if the CSS port needs restructured selectors rather than renamed ones, decide then whether a utility layer is in scope (R21).
- **`plans/step-cli-to-ca-lib-swap.md`.** Removed 2026-09-02. The swap it described had already landed: `backend/stepca/` (`client.go`, `bootstrap.go`, `issue.go`, `revoke.go`) wraps `github.com/smallstep/certificates/ca`, `handlers/stepcli.go` is gone, no `exec.Command("step", ...)` remains under `backend/`, and the Dockerfile no longer installs the `step` binary.
