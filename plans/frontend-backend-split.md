...
| [12. Open questions](#12-open-questions) | Q1 to Q3 answered 2026-08-13; Q4 to Q7 reopened by the two-container answer |
...
```
v1  GET  /api/v1/certificates/{id}  auth=required  role=viewer  tag=certificates  csrf=no  ratelimit=no  templateRouteRetired=false
```
...
9. `.gitattributes` merge driver, `make openapi`, `make hooks`, and a **standalone** drift gate plus `oasdiff` gate, which Phase 2 relocates into the `client` job. 7.2's same-job requirement becomes true when the `client` job exists. Until then a standalone gate is strictly better than none.
...
3. **The eight overrides, plus one new one.** `-config`, `-mail` and `-le` are unaffected. `-image` needs a second `image:` pin, which is the one edit that silently defeats the whole override if missed. `-oidc` needs three: `TRUST_PROXY` defaults to false there, `TRUSTED_PROXY_CIDRS` defaults empty (which with the first flipped is now a `log.Fatalf`), and `OIDC_REDIRECT_URL` points at the backend on a path that does not match D5 rule 10's published origin. `-fatals`, `-nodeps` and `-fingerprint` need **no edits**, because the new `compose.e2e-noweb.yml` gives `step-ui-web` a never-enabled profile and `scenario.sh` composes it into every scenario, so nginx never exists for the `infra` project.
