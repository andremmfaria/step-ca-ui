...
Status: revision 9, 2026-09-02. Written 2026-08-13 against `main` at `a549750`, revised through two four-phase reviews, and **partly executed: Phases 0 and 1 are complete and merged** (Phase 1 merged 2026-08-15, last code commit `304f880`), so the decisions they tested are now measured rather than proposed. **All seven open questions are answered as of 2026-08-15, so nothing in this plan is blocked on a decision.** **Phase 2, the client package pipeline, began 2026-09-02.** The e2e track is active, not paused, with a scope correction recorded in Section 10. Revision 9 also folds in a 2026-09-02 maintenance pass (Go 1.26.8, Vite 8.2.2, GitHub Actions pinned by version tag rather than commit SHA, npm added to Dependabot for `frontend`, `clients/ts` and `test/e2e`) and removes the stale `plans/step-cli-to-ca-lib-swap.md` draft, recorded in Section 12.
...
| [8. Phases](#8-phases) | Phases 0, 1, 2, 3a, 3b, 3c, 4 to 9, all on `main`. 0 and 1 are complete, 2 began 2026-09-02, only Phase 9 is irreversible |
