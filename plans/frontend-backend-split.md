...
| `ui` project (4 browser companions) | **Write from scratch.** Not yet written, which is the good case |
...
6 of the 34 written IDs survive untouched, 27 need their bodies rewritten, 1 (STATIC-01) is retired in Phase 9, and the largest untouched block, the 9 `infra` IDs, is not written yet. The rewrite is a body rewrite, not a re-derivation: `plans/e2e-tests.md` already states what each test asserts.
